...
export async function GET(
  ...
  if (memberError) {
    return NextResponse.json({ error: "학생 조회 실패" }, { status: 500 });
  }
...
export async function POST(
  ...
  if (memberError) {
    await admin.from("profiles").delete().eq("id", userResult.user.id);
    await admin.auth.admin.deleteUser(userResult.user.id);
    return NextResponse.json({ error: "방 멤버 등록 실패" }, { status: 500 });
  }

...
export async function getTeacherRoomStatsView(roomId: string): Promise<TeacherRoomStatsView> {
  ...
  const [profilesResult, questionRowsResult, roomScoresResult] = await Promise.all([
    supabase.from("profiles").select("id, name").in("id", memberIds).eq("role", "student"),
    supabase
      .from("questions")
    ...
    supabase
      .from("room_student_scores")
      .select("student_id, total_score")
      .eq("room_id", roomId)
      .in("student_id", memberIds),
  ]);

  const nameMap = new Map((profilesResult.data ?? []).map((p) => [p.id, p.name]));
  const questionRows = questionRowsResult.data ?? [];
  const scoreByStudentId = new Map(
    (roomScoresResult.data ?? []).map((row) => [row.student_id, row.total_score ?? 0]),
  );
